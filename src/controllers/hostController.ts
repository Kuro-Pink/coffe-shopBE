...
class HostController {
  ...
  toggleProductAvailability = catchAsync(async (req: Request, res: Response) => {
    ...
    res.status(200).json(ApiResponse.success(product, 'Product availability updated successfully'));
  });
  ...
  getTables = catchAsync(async (req: Request, res: Response) => {
    ...
    res.status(200).json(ApiResponse.success(tables, 'Tables retrieved successfully'));
  });

  // Get table by ID
  getTableById = catchAsync(async (req: Request, res: Response) => {
    const table = await tableService.getTableById(req.params.id);
    res.status(200).json(ApiResponse.success(table, 'Table retrieved successfully'));
  });
  ...
  createTable = catchAsync(async (req: Request, res: Response) => {
    ...
    res.status(201).json(ApiResponse.success(table, 'Table created successfully', 201));
  });

  // Update table
  updateTable = catchAsync(async (req: Request, res: Response) => {
    const table = await tableService.updateTable(req.params.id, req.body);
    res.status(200).json(ApiResponse.success(table, 'Table updated successfully'));
  });
  ...
  updateTableStatus = catchAsync(async (req: Request, res: Response) => {
    ...
    res.status(200).json(ApiResponse.success(table, 'Table status updated successfully'));
  });
  ...
  getUnpaidOrdersByTable = catchAsync(async (req: Request, res: Response) => {
    ...
    if (!table.currentSession) {
      return res.status(200).json(ApiResponse.success([], 'No active session for this table'));
    }

    // ✅ Controller truyền session sang orderService
    const orders = await orderService.getUnpaidOrdersBySession(tableId, table.currentSession);

    res.status(200).json(ApiResponse.success(orders, 'Unpaid orders retrieved successfully'));
  });

  // Delete table
  deleteTable = catchAsync(async (req: Request, res: Response) => {
    await tableService.deleteTable(req.params.id);
    res.status(200).json(ApiResponse.success(null, 'Table deleted successfully'));
  });

  // Regenerate QR code
  regenerateQRCode = catchAsync(async (req: Request, res: Response) => {
    const table = await tableService.regenerateQRCode(req.params.id);
    res.status(200).json(ApiResponse.success(table, 'QR code regenerated successfully'));
  });
  ...
  getTableStats = catchAsync(async (req: Request, res: Response) => {
    ...
    res.status(200).json(ApiResponse.success(stats, 'Table statistics retrieved successfully'));
  });
  ...
  getTodayStats = catchAsync(async (req: Request, res: Response) => {
    ...
    res.status(200).json(ApiResponse.success(stats, 'Today statistics retrieved successfully'));
  });
}
